...
export function ColumnCard({
  ...
  return (
    <div
      className={`
        group flex items-center gap-3 p-3 bg-white border border-gray-200 rounded-lg
        transition-all duration-200
        ${isDragging ? 'opacity-50 shadow-lg' : 'hover:border-gray-300 hover:shadow-sm'}
      `}
    >
      {/* Drag Handle */}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-col gap-1">
          <p className="text-xs text-gray-500">
            Status: <span className="font-mono text-gray-700">{column.status}</span>
          </p>
          ...
          {column.gmailLabelId ? (
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-400">No label mapped</span>
            </div>
          )}
        </div>
      </div>
